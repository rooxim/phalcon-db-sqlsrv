...
//                }
    ...
    public function addColumn($tableName, $schemaName, \Phalcon\Db\ColumnInterface $column)
    {
        ...
        if ($column->isAutoIncrement()) {
            $sql .= ' IDENTITY NOT NULL';
        }

        if (!$column->isAutoIncrement() && $column->isNotNull()) {
            $sql .= ' NOT NULL';
        }
        if ($column->isFirst()) {
            $sql .= ' FIRST';
    ...
    public function modifyColumnDefaultValue($tableName, $schemaName, \Phalcon\Db\ColumnInterface $column, \Phalcon\Db\ColumnInterface $currentColumn = null)
    {
        $defaultValue = $column->getDefault();
        ...
        return 'ALTER TABLE '.$this->prepareTable($tableName, $schemaName).' ADD CONSTRAINT DF_'.$column->getName().' DEFAULT '.$defaultValue.' FOR ['.$column->getName().']';
    ...
    {
        return 'DROP INDEX '.$this->prepareTable($tableName, $schemaName).'.['.$indexName.']';
    }
    /**
    ...
    {
        $sql = 'ALTER TABLE '.$this->prepareTable($tableName, $schemaName).' ADD CONSTRAINT ['.$reference->getName().'] FOREIGN KEY ('.$this->getColumnList($reference->getColumns()).') REFERENCES '.$this->prepareTable($reference->getReferencedTable()).'('.$this->getColumnList($reference->getReferencedColumns()).')';
            ...
            foreach ($definition['indexes'] as $index) {
                ...
                if ($indexName == 'PRIMARY') {
                    ...
                    if (!empty($indexType)) {
                        switch($indexType){
                            case 'UNIQUE':
                                $indexSql = 'CONSTRAINT'.' ['.$indexName.']  UNIQUE('.$this->getColumnList($index->getColumns()).')';
                                break;
                        }
                    } else {
    ...
    {
        $sql = "SELECT ind.name as index_name,col.name as col_name,is_unique,is_primary_key
...
WHERE t.name = '".$table."'";
    ...
    {
        $sql = 'SELECT TABLE_NAME,COLUMN_NAME,CONSTRAINT_NAME, TABLE_SCHEMA as REFERENCED_TABLE_SCHEMA,TABLE_NAME as REFERENCED_TABLE_NAME,COLUMN_NAME as REFERENCED_COLUMN_NAME FROM INFORMATION_SCHEMA.KEY_COLUMN_USAGE WHERE ';
        if ($schema) {
...
//            }
    ...
    {
        //var_dump($table,$schema,$alias,$escapeChar);die;
        $tableQuery = '';
        if($schema){
            $tableQuery .= '['.$schema.'].';
        }
